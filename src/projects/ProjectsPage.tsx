    ...
    <main className="page-shell relative isolate flex min-h-dvh flex-col overflow-hidden">
      ...
      <header className="fixed inset-x-0 top-0 z-20 px-4 pt-4 sm:px-6">
        <nav className="mx-auto flex max-w-6xl items-center justify-between rounded-full border border-white/10 bg-zinc-950/50 px-4 py-3 shadow-2xl shadow-black/10 backdrop-blur-xl sm:px-5">
          ...
          <a className="min-h-10 content-center text-sm text-white/65 transition-colors duration-200 hover:text-white focus-visible:text-white" href="/projects" aria-current="page">
            Projects
          </a>
        </nav>
      </header>
      ...
      <section className="flex flex-1 items-center justify-center px-4 pb-4 pt-24 sm:px-6 sm:pt-28">
        <div className="w-full max-w-5xl text-center">
          ...
          <h1 className="mt-6 text-4xl font-semibold tracking-tight text-white sm:text-6xl md:text-7xl">Projects</h1>
          <p className="mx-auto mt-6 max-w-xl text-base leading-7 text-white/65 sm:text-lg">Small tools, made with care.</p>

          <div className="mx-auto mt-10 grid max-w-4xl gap-3 md:grid-cols-3">
            {projects.map((project) => (
              <a className="group min-h-40 rounded-2xl border border-white/10 bg-white/[0.035] p-6 text-left backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-white/25 hover:bg-white/[0.07] focus-visible:-translate-y-1 focus-visible:border-white/40" href={project.url} key={project.name} rel="noreferrer" target="_blank">
                <span className="flex items-start justify-between gap-3 text-lg font-medium text-white">
